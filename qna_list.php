...
$sSql = " select count(*) as scnt from qna " . $sqry;
...
<section2>
  ...
  <form method="post" action="qna_list.php" style="float:right">
	  	<div class="input-group mt-3 mb-3" >
		  <div class="input-group-prepend">
		    <select class="btn btn-outline-secondary dropdown-toggle" name="keyfield" data-toggle="dropdown">
		      <option class="dropdown-item" value="id">Search Name</option>
...
$con = mysqli_connect("localhost", "root", "", "test");
$sql = "select * from qna " . $sqry . " order by num desc";
...
for ($i = $start; $i < $start + $scale && $i < $total_record; $i++) {
  ...
  <tbody>
      <tr>
        <td><?= $number ?></td>
        <td ><a href="qna_view.php?num=<?= $num ?>&page=<?= $page ?>" style="color:black;"><?= $subject ?></a></td>
        <td><?= $id ?></td>
        <td><?= $file_image ?></td>
        <td><?= $regist_day ?></td>
        <td><?= $hit ?></td>
...
if ($total_page >= 2 && $page >= 2) {
  $new_page = $page - 1;
  echo "
  <li><a href='qna_list.php?page=$new_page'>◀ 이전</a> </li>";
} else {
  echo "<li>&nbsp;</li>";
}
...
?>
				<li>
					<button onclick="location.href='qna_form.php'" class="btn btn-primary" style="color:white; float:right;">글쓰기</button>
				</li>
			</ul>
      </div>
	</div> <!-- board_box -->
