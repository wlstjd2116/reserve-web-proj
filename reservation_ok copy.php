...
$sql = "select house_id,rs_year, rs_month, rs_day from tb_house_reservation where (house_id = " . $_POST["insert_houseid"] . "and rs_date =" . $date . " )";
...
if (empty($row["house_id"]) == false || $timenow > $timetarget) { ?>
   		...
   		<?php } else {
        if ($_POST["total_man"] <= $row2["max_people"]) {
    $sql =
      " insert into tb_house_reservation(user_id, house_id, rs_year, rs_month, rs_day, tot_man, insert_date, rs_date) values('" .
      $_SESSION["userid"] .
      "','" .
